...
public class Java8Stream {
    ...
    public Map<String, Optional<People>> getMaxAgeGroupBySex(List<People> peoples) {
        return peoples.parallelStream()
            .collect(Collectors.groupingBy(People::getSex, Collectors.maxBy(Comparator.comparing(People::getAge))));
    }

    public Map<String, People> getPeopleMapByName(List<People> peoples) {
        return peoples.parallelStream()
            .collect(Collectors.toMap(People::getName, Function.identity(), (newValue, oldValue) -> newValue));
    ...
    public List<People> getSumAmountGroupBySex(List<People> list) {
        ...
        list.stream().collect(Collectors.groupingBy(People::getSex)).forEach((name, fooListByName) -> {
    ...
    public static Map<String, List<People>> buildRecordMapBrief(List<People> records) {
        return records.stream()
            .collect(Collectors.groupingBy(record -> buildRecordKey(record.getAge(), record.getSex())));
    }
    ...
    public String jointStringArray(List<String> letters) {
        ...
        // String result = letters.stream().reduce("", (partialString, element) -> partialString.toUpperCase() +
        // element.toUpperCase());
    ...
    private Map<Integer, String> filterMap(Map<Integer, String> map, String filterName) {
        Map<Integer, String> collect = map.entrySet().stream().filter(x -> filterName.equals(x.getValue()))
        ...
        String result = map.entrySet().stream().filter(x -> filterName.equals(x.getValue())).map(x -> x.getValue())
    ...
    public static void main(String[] args) {
        ...
        System.out.println(j8.jointStringArray(list));
